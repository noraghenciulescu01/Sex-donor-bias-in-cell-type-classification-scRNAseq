...
def train_clf_and_predict(X, y, sex_labels, individual_labels, proportion_female, classifier='knn', k = 30):
    ...
    gss = GroupShuffleSplit(n_splits=1, test_size=0.2, random_state=60)
...
def match_male_female_donors(adata, covariates):
    '''
    Filters adata to contain only matched male and female donors
    based on categorical covariates.
    '''
    # create donor table, extract male and female donors
    donor_table = adata.obs.groupby('donor_id').agg({
        'sex': 'first',
        **{cov: 'first' for cov in covariates}
    }).reset_index()
    
    male_donors = donor_table[donor_table['sex'] == 'male'].copy()
    female_donors = donor_table[donor_table['sex'] == 'female'].copy()
    
    
    matched_male_ids = []
    matched_female_ids = []

    # match on categorical covariates
    for idx, male_row in male_donors.iterrows():
        candidates = female_donors.copy()
        for cov in covariates:
            candidates = candidates[candidates[cov] == male_row[cov]]
        if not candidates.empty:
            # pick first match
            female_match = candidates.iloc[0]
            matched_male_ids.append(male_row['donor_id'])
            matched_female_ids.append(female_match['donor_id'])
            # remove matched female to prevent reuse
            female_donors = female_donors[female_donors['donor_id'] != female_match['donor_id']]

    matched_donor_ids = matched_male_ids + matched_female_ids
    print(f"Matched {len(matched_male_ids)} male-female donor pairs "
          f"({len(matched_donor_ids)} donors total).")

    # filter adata, return
    matched_adata = adata[adata.obs['donor_id'].isin(matched_donor_ids)].copy()
